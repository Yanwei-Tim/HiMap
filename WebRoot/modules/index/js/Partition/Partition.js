    ...
    function showDistrict(nlevel) {
    	_MapApp.Tools.sendAjax(HiMapConfig.HOSTNAME+"mapindex/getLayerData?lid=6&nlevel="+nlevel,function(result){
            ...
            for (var iIndex = 0; iIndex < districtarr.length; iIndex++) {
                ...
                var pOverlay = _MapApp.showPolygon({strcoords:pFeat.coordinate, color:"red", weight:1.5, opacity:0.5, fillcolor:overlayColor});
                var pTitle = _MapApp.showTitle({name:pFeat.shortname,strcoords:pOverlay.centroid(),color:'white',bgcolor:'green',borderSize:0});
