...
define(["map/Config","map/IHiMap","tool/tools","overlay/HiIcon",
		...
		"overlay/HiRect","overlay/HiTitle"], 
		function(CONFIG,HiMap,Tools,HiIcon,HiMarker,HiMBR,HiOverlay,
				 HiPoint,HiPolygon,HiPolyline,HiRect,HiTitle) {
	//空方法
    function noop(){}
    ...
    HiMap.prototype.showDevice = function (device) {
    	device = {devicetype:'13',devicename:'ddddd',videotype:'1',ctrlflag:'0',devicestate:'1',longitude:120.38019,latitude:36.08671};
		//device = {devicetype:'10',devicename:'诱导屏',vmstype:'0',direction:'7',devicestate:'',longitude:120.38019,latitude:36.08671};
		...
		for(var i = 0;i < iconparams.length; i++){
			...
			var paramvalues = [];
			var bIsExist = false;			//标记，当括号中有值时，为true
			...
			if(paramname.indexOf("(")>0){
				...
				for (var j = 0 ; j < paramvalues.length ; j++ ){		//逐个比较是否在括号中
					if(device[paramname] ==	paramvalues[j]){		//如果在括号中
						bIsExist = true;					//改为TRUE
						break;
					}
				}
				if(!bIsExist){
		...
		var deviceimgurl = "vendor\\himap\\icons\\device\\"+imgpath+".png";
		var strcoords = device.longitude+','+device.latitude;
		//alert(deviceimgurl);
		
		this.showMonitor({strcoords:strcoords,title:device.devicename,imgurl:deviceimgurl});
	};
	...
	//problem
		...
		var tempTitle = new HiTitle({name:params.title});
		tempTitle.set({name:params.title,color:params.titlecolor,bgColor:params.titlebgcolor});
    ...
    */
	HiMap.prototype.showPolyline = function (params) {
		...
		var tempPolyline =  new Polyline(params.strCoords,params.color, params.weight, params.opacity, params.arrow);
	...
	HiMap.prototype.showRect = function (params) {
		...
		var tempRectangle =  new Rectangle(params.strCoords,params.color, params.weight, params.opacity, params.fillcolor);
		...
		if(params.centable == true){
			this.centerAtMBR(tempRectangle.getMBR());
		}
	...
	HiMap.prototype.showPolygon = function (params) {
		...
		var tempPolygon = new Polygon(params.strCoords,params.color, params.weight, params.opacity, params.fillcolor);
	...
	HiMap.prototype.showCircle = function (params) {
		...
		var tempCircle = new Circle(params.strCoords,params.color, params.weight, params.opacity, params.fillcolor);
